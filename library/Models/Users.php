...
class Users extends AbstractModel
{
    ...
    public function getValidationData(): Validator
    {
        ...
        $validate = new Validator($token, 10);

        return $validate
            ->validateIssuer($this->get('issuer'))
            ->validateAudience($this->getTokenAudience())
            ->validateIssuedAt($this->getTokenTimeIssuedAt())
            ->validateNotBefore($this->getTokenTimeNotBefore())
            ->validateExpiration($this->getTokenTimeExpiration())
            ->validateId($this->get('tokenId'));
    }
